...
if [[ "${CONFIRM_FR5_HARDWARE:-}" != "YES" ]]; then
  echo "Refusing to start FR5 ServoJ control." >&2
  echo "Verify the cell, E-stop, payload, collision model and TCP in RViz, then run:" >&2
  echo "  CONFIRM_FR5_HARDWARE=YES $0 $*" >&2
  exit 2
fi
